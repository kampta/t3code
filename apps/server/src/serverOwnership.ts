...
export class ServerOwnershipConflictError extends Schema.TaggedError<ServerOwnershipConflictError>()(
...
export class ServerOwnershipStateError extends Schema.TaggedError<ServerOwnershipStateError>()(
...
const tryAcquireOwnershipLease = (input: {
  ...
  Effect.tryPromise({
    try: async () => {
      ...
      try {
        database = new (await import("node:sqlite")).DatabaseSync(input.leasePath);
