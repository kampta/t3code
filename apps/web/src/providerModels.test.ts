...
describe("resolveSelectableProvider", () => {
  ...
  it("does not treat a non-Codex driver in the default Codex slot as preferred", () => {
    const providers = [
      provider({ driver: "claudeAgent", instanceId: "codex", status: "ready" }),
      provider({ driver: "codex", instanceId: "codex_work", status: "ready" }),
    ];

    expect(resolveSelectableProvider(providers, null)).toBe("codex");
  });
